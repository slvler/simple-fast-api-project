...
def store(request: BlogRequest, db: db_dependency):
    try:
        new_blog = Blog(
            name=request.name,
...
def show(blog_id: int, db: db_dependency):
...
def update(blog_id: int, request: BlogRequest, db: db_dependency, ):
    blog = db.query(Blog).filter(Blog.id == blog_id).first()
    if blog is not None:
...
def delete(blog_id: int, db: db_dependency):
    blog = db.query(Blog).filter(Blog.id == blog_id).first()
    if blog is not None:
        db.delete(blog)
        db.commit()
